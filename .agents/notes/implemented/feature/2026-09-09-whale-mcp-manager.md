...
- **Settings-driven mounts.** The `mcp` namespace holds one array of server entries (stdio and streamable-http, field-for-field compatible with the `claude_desktop_config.json` shape). The manager watches the section and reconciles the mount set: one bridge fiber per enabled entry, mounted with `failOnStartupError: true` so a bad server is a visible `failed` status instead of a silently empty tool set. Edits diff by entry `id` plus a fingerprint over the bridge-consumed fields — untouched servers never restart. A server that dies after a successful start is the bridge's reconnect supervisor's job; a server that fails to START is the user's to retry via the tab's Restart button (`mcpStatus.restart`), which is the honest split because a startup failure usually needs a human's fix. Startup is also BOUNDED: the manager hands each mount a startup deadline (default 30 s) that the bridge enforces on the initial connection + tool sync, and the manager bounds its own await as a net — one hanging external server must never hold the serialized reconcile queue (and every later server, edit, or restart behind it), so a timed-out mount fails visibly and the rest of the section reconciles.
- **Status is pulled, not pushed.** `mcpStatus.list()` merges manager state (connecting/connected/failed/disabled) with tool names enumerated from the live registry at call time. Tools are attributed to servers by their CONFIGURED literal `mcp__<name>__` prefix, longest match first — never by parsing the name at the first `__`, which would misattribute a legal server named `a__b` to a server `a`; the approval fence uses the same attribution. Internal cells track live servers only (removed/disabled entries leave no record; an unchanged enabled entry keeps its visible failure). No push channel, no second lifecycle truth; the tab refetches after saves at +600 ms/+2500 ms to let `connecting` settle.
