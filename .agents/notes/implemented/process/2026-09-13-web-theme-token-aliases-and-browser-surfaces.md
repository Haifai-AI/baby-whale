...
`--dsw-shadow-lv1/2/3` become contact-plus-ambient pairs (a tight small-offset shadow under a wide large-blur one), with a dark-palette block that deepens the alphas and swaps in a light hairline. [The visual-world replacement](2026-09-14-web-client-apple-visual-world.md) later gave every step the same hairline-plus-pair shape and a dark override for all three; the pairing decision recorded here is unchanged.
...
Every `var(--dsw-*)` reference in `packages/client` and `apps/web` now resolves against a declaration, and a refresh of the same scan reports only the two runtime-injected geometry variables (`--dsh-composer-height` from the seat observer, `--dsh-boot-arc` from the boot page's own loop). The alias inventory this note established survives the [visual-world replacement](2026-09-14-web-client-apple-visual-world.md), which re-values the aliases and adds the closed neutral ramp and accent beneath them; the browser-surface sheet, the focus-ring indirection, and the scrollbar rebinding contract are unchanged. The focus ring and selection color are one affordance in every theme, and the elevation ladder reads as height in both palettes.
