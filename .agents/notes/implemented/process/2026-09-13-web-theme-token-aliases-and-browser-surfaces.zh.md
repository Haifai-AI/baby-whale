...
`--dsw-shadow-lv1/2/3` 改为接触阴影加环境阴影的配对（一个紧凑小偏移的阴影垫在一个大模糊的宽阴影之下），并增加深色色板块，加深 alpha 并换用浅色发丝描边。[视觉世界替换](2026-09-14-web-client-apple-visual-world.zh.md) 随后让三个层级都采用同样的"发丝线加成对阴影"结构，并为三者补齐深色覆盖；此处记录的配对决策未变。
...
`packages/client` 与 `apps/web` 中每一处 `var(--dsw-*)` 引用现在都能解析到声明；重跑同一扫描只会报出两个由运行时注入的几何变量（`--dsh-composer-height` 来自座位观察器，`--dsh-boot-arc` 来自启动页自身的循环）。焦点环与选中色在每种主题下都是同一个可供性，层级阶梯在两种色板下都读得出高度。本文确立的别名清单在[视觉世界替换](2026-09-14-web-client-apple-visual-world.zh.md)之后依然成立——该变更重新赋值别名，并在其下新增封闭中性色阶与强调色；浏览器表面样式表、焦点环间接层与滚动条重绑定契约均未改变。
