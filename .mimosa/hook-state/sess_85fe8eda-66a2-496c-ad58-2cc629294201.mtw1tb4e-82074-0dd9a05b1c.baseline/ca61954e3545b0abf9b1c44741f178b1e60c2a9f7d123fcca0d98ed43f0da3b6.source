...
class FakeFileSystem extends FileSystem {
  ...
  override async writeBytes(target: FsTarget, bytes: Uint8Array, _expected?: FsWriteIntent): Promise<FsBinaryWriteOutcome> {
    const before = this.files.get(target.targetKey)
    this.files.set(target.targetKey, Buffer.from(bytes).toString('base64'))
    return { operation: before !== null ? 'update' : 'create', version: FsVersion('v2'), size: bytes.byteLength }
  }
