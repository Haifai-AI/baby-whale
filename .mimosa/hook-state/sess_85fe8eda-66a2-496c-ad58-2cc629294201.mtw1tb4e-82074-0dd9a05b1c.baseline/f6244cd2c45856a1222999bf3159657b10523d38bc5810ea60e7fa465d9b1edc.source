...
/** First-party harness-shipped skills: same tier as bundled, just below it. */
const HARNESS_SKILL_RANK = 610
...
export const Config: Schema<Config> = z.object({
  ...
  harnessSkillDir: z.string().default(''),
})
...
export class FileSystemSkillProvider implements SkillProvider {
  ...
  private readonly harnessSkillDir: string | undefined
  ...
  constructor(
    ...
    const harnessSkillDir = config.harnessSkillDir !== undefined && config.harnessSkillDir !== ''
      ? config.harnessSkillDir
      : (this.includeDefaultRoots ? process.env.DSH_HARNESS_SKILL_DIR : undefined)
    this.harnessSkillDir = harnessSkillDir === undefined || harnessSkillDir === ''
      ? undefined
      : resolve(harnessSkillDir)
  }
  ...
  private async roots(cwd: string | undefined): Promise<SkillRoot[]> {
    ...
    if (this.harnessSkillDir !== undefined) {
      roots.push({ path: this.harnessSkillDir, source: 'harness', rank: HARNESS_SKILL_RANK, trustedHost: true })
    }
    return roots
  }
}
