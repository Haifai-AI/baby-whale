...
/** The app's bundled skill directory, shared by every preset through the host `skill-filesystem` row. */
const SHIPPED_SKILL_ROOT = fileURLToPath(new URL('../config/skills/', import.meta.url))
/** The harness repo's own `.agents/skills`, shipped as a first-party always-on skill layer. */
const HARNESS_SKILL_ROOT = fileURLToPath(new URL('../../../.agents/skills/', import.meta.url))
...
async function composeProfile(
  ...
  // The bundled skill root rides the same pattern: the launcher is the only
  // component that knows where its own config lives, and a single shared dir
  // makes every shipped skill visible to EVERY preset and to the Settings →
  // Skills manager as one global, toggleable list. (Shipped agent presets
  // self-wire their root inside the agent-presets package.)
  if (rows.has('skill-filesystem')) {
    composedOverlays.push({
      id: 'skill-filesystem',
      config: {
        ...(rows.get('skill-filesystem')?.config ?? {}) as Record<string, unknown>,
        bundledSkillDir: SHIPPED_SKILL_ROOT,
        ...(existsSync(HARNESS_SKILL_ROOT) ? { harnessSkillDir: HARNESS_SKILL_ROOT } : {}),
      },
    })
  }
  process.env.DSH_BUNDLED_SKILL_DIR = SHIPPED_SKILL_ROOT
  if (existsSync(HARNESS_SKILL_ROOT)) process.env.DSH_HARNESS_SKILL_DIR = HARNESS_SKILL_ROOT
...
export async function runProfile(options: RunProfileOptions): Promise<{ ctx: Context; shutdown: ProcessShutdown }> {
  // Office code-execution support (venv + document libraries) is prepared in
  // the background and never blocks boot.
