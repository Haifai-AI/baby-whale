...
function createFixtureWorld(options: FixtureOptions): FixtureWorld {
  ...
  const api: ApiProxy = {
    artifacts: {
      list: request => ok(request, { artifacts: [] }),
      preview: request => ok(request, { size: 0 }),
    ...
    skills: {
      list: (request) => {
        ...
        return ok(request, {
          skills: [
            { name: 'fixture-demo', description: 'fixture 技能样本', whenToUse: '仅供 UI 目录渲染验收', source: 'bundled', modelInvocable: true, userDisabled: false },
            { name: 'fixture-user-only', description: 'fixture 仅用户技能样本', source: 'user-dsh', modelInvocable: false, userDisabled: false },
          ],
        })
      },
    },
    ...
    uploads: {
      workspaceFile: () => Promise.resolve(new Response('{}')),
    },
    downloads: {
