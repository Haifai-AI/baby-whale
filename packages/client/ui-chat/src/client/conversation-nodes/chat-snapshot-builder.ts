...
/**
 * Session-bootstrap plumbing that renders as an orphan row when it predates
 * the opening prompt: the auto-dispatched permission command card, kinds the
 * renderer has no view for, and plugin context injections (user-approval
 * receipts and the like). Recall context is exempt: it is conversational
 * continuity.
 */
function isBootstrapNoise(node: ChatConversationViewNode): boolean {
  const kind = (node as ChatNode).kind
  if (kind === 'command') {
    const data = (node as ChatNode<'command'>).data as { command?: { name?: unknown } }
...
function chatOrderKeys(nodes: readonly ChatConversationViewNode[]): readonly string[] {
  const visible = orderedVisibleChatNodes(nodes)
