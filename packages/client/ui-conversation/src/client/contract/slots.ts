...
export interface ComposerAttachmentsOwnerProps {
  ...
  /** Upload one dropped batch of non-image files into the session workspace. */
  onUploadFiles?: ((files: readonly File[]) => void) | undefined
...
declare module '@deepseek-ai/dsh-client-ui-slots' {
  interface SlotMap {
    ...
    /**
     * Keyed per-tool output seat inside the Details panel, dispatched by wire
     * Tool name — the details-panel analogue of `tool.call.toolview`. An
     * unclaimed key falls back to the legacy single
     * `conversation.details.tool` occupant, so registering here is additive
     * for the key's own tool and a no-op for every other tool.
     */
...
export interface ChatNodeOwnerProps {
  /** Open the Details panel on one Tool call (the chat row affordance). */
  openDetails?: (target: { callId: string; toolName: string }) => void
