...
export function InputBar({
  ...
  // Workspace uploads (drop/paste of non-image files): sequential POSTs to
  // the host-only intake channel; each success appends a `[file: …]` mention
  // token to the draft so the model receives an unambiguous workspace path.
    ...
    void (async () => {
      for (const file of files) {
        ...
        try {
          ...
          const outcome = await response.json() as { path: string }
          showToast(t('file.uploaded', { name: outcome.path }))
          if (keyboard !== undefined && !machineBusy && !locked) {
            const draftText = keyboard.snapshot.draft
            const token = `[file: ${outcome.path}] `
            const next = draftText.length === 0 ? token : `${draftText}\n${token}`
            keyboard.setDraft(next, { start: next.length, end: next.length, insertedLength: token.length })
          }
        } catch {
          showToast(t('file.uploadFailed', { name: file.name }))
        }
      }
    })()
  }, [keyboard, locked, machineBusy, sessionId, showToast, t])
