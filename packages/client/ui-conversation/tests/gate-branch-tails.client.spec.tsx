...
describe('render branch tails', () => {
  ...
  it('DetailsPanel resolves a nested run_code leaf to its full logged args and output', () => {
    ...
    // frozen block to the Tool-owned details seat. The keyed toolview slot is
    // the terminal render; the probed fallback pair (tool + toolview) is two
    // calls for the same block, so assert on the LAST call — the toolview.
    expect(view.getByText('read')).toBeTruthy()
    expect(view.getByTestId('tool-details-seat')).toBeTruthy()
    expect(owners.length).toBeGreaterThanOrEqual(1)
    expect(owners.at(-1)?.block).toMatchObject({
