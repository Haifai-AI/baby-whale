...
describe('FishLogo', () => {
  it('renders the Baby Whale mark image at the requested size', () => {
    const { container } = render(<primitives.FishLogo />)
    const img = container.querySelector('img')!
    expect(img.getAttribute('src')).toBe('/baby-whale.png')
  ...
  it('scales the mark with the size prop', () => {
    const { container } = render(<primitives.FishLogo size={34} />)
    const img = container.querySelector('img')!
