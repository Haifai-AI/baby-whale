...
/** Minimal slice of the office.runtime status payload the card renders. */
 ...
 */
export function RuntimeSetupCard({ wide, t }: RuntimeSetupCardProps): JSX.Element | null {
  const [status, setStatus] = useState<RuntimeStatus | undefined>(undefined)
  const [busy, setBusy] = useState(false)

  const refresh = useCallback(async () => {
    try {
      const response = await fetch('/api/office.runtime')
      if (response.ok) setStatus(await response.json() as RuntimeStatus)
    } catch {
  ...
  const start = (): void => {
    ...
    void (async () => {
      try {
        const response = await fetch('/api/office.runtime.install', { method: 'POST' })
        const value = response.ok ? await response.json() as { install: RuntimeStatus['install'] } : undefined
        if (value !== undefined && status !== undefined) setStatus({ ...status, install: value.install })
      } catch {
