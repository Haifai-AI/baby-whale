...
export const CLIENT_SLOT_API: readonly ClientSlotEntry[] = [
  ...
  },
  {
    ...
    keyDomain: 'fixed by the owner\'s key table { [Kind in ChatNodeKind]: { node: ChatNode<Kind> } }, already taken: assistant-step, command, command-input, compaction, context, manual-compaction, model-retry, steering, tool-call, turn-error, turn-max-tokens, turn-tail, unknown, user, whale-task-board, workflow-run',
    hookContext: 'string',
    ...
    occupants: [
      ...
      'client-ui-whale-tasks TaskBoardNode key \'whale-task-board\'',
  ...
  {
    key: 'conversation.details.fileview',
    kind: 'single',
    scope: 'session',
    summary: 'Whole-panel deliverable file preview: the produced-file cards\' Preview gesture routes here instead of a modal, so the chat stays live beside the rendered file.',
    doc: 'Whole-panel deliverable file preview: the produced-file cards\' Preview\ngesture routes here instead of a modal, so the chat stays live beside\nthe rendered file. One occupant; ui-deliverables owns the renderer.',
    registerOptions: [],
    ownerProps: [
    ...
    occupants: [
      'client-ui-deliverables FilePreviewPane',
    ],
    replaceRisk: 'shadows-shipped-ui',
    example: 'return {\n  inject: [\'slots\'],\n  apply(ctx) {\n    ctx.slots.inject(\'conversation.details.fileview\', () => ctx.slots.register(\n      { name: \'conversation.details.fileview\' },\n      () => React.createElement(\'div\', null, \'hello\'),\n    ))\n  },\n}',
  ...
  {
    key: 'conversation.details.toolview',
    kind: 'keyed',
    scope: 'session',
    summary: 'Keyed per-tool output seat inside the Details panel, dispatched by wire Tool name — the details-panel analogue of `tool.call.toolview`.',
    doc: 'Keyed per-tool output seat inside the Details panel, dispatched by wire\nTool name — the details-panel analogue of `tool.call.toolview`. An\nunclaimed key falls back to the legacy single\n`conversation.details.tool` occupant, so registering here is additive\nfor the key\'s own tool and a no-op for every other tool.',
    registerOptions: [
      {
        name: 'key',
        requirement: 'required',
    ...
    declaredBy: 'an entry in \'details\' (client-ui-conversation), so it exists while that entry is mounted',
    occupants: [
      'client-ui-whale-artifact DetailsArtifactView',
    ],
    replaceRisk: 'none',
    example: 'return {\n  inject: [\'slots\'],\n  apply(ctx) {\n    ctx.slots.inject(\'conversation.details.toolview\', () => ctx.slots.register(\n      { name: \'conversation.details.toolview\', key: \'<one key the owner dispatches>\' },\n      () => React.createElement(\'div\', null, \'hello\'),\n    ))\n  },\n}',
  ...
  },
  {
    ...
    occupants: [
      ...
      'client-ui-whale-artifacts ArtifactsView id \'whale-artifacts\'',
    ],
    replaceRisk: 'none',
  ...
  },
  {
    ...
    occupants: [
      'client-ui-settings-mcp McpSettingsTab id \'mcp\'',
  ...
  },
  {
    ...
    occupants: [
      ...
      'client-ui-whale-artifact OfficeArtifactCard',
