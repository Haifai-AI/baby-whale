...
/**
 * Outcome of a full-file binary write. Binary content has no text diff basis,
 * so the outcome carries the byte size instead of before/after text.
 */
export interface FsBinaryWriteOutcome {
  ...
  /** Opaque version of the file after the write. */
  version: FsVersion
  /** Byte length of the content written. */
  size: number
}
