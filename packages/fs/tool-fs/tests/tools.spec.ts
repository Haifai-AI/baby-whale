...
class FakeFs extends FileSystem {
  ...
  override async writeBytes(target: FsTarget, bytes: Uint8Array, expected?: FsWriteIntent): Promise<FsBinaryWriteOutcome> {
    this.throwIfArmed()
    this.writeIntents.push(expected)
    this.files.set(target.targetKey, Buffer.from(bytes).toString('base64'))
    return { operation: 'create', version: FsVersion('v2'), size: bytes.byteLength }
  }
...
describe('sandbox escalation API (write/edit)', () => {
  ...
  class SandboxingFakeFs extends FakeFs {
    ...
    override async writeBytes(
      ...
      this.stamped.push(sandboxPolicy)
      return super.writeBytes(target, bytes, expected)
    }
  ...
  it('a plain write stamps the default mode with the calling session root', async () => {
    ...
    expect(fs.stamped).toEqual([{ mode: 'workspace-write', egress: 'deny', workspaceRoot: resolve('/session-project') }])
  })
  ...
  it('a standing session override folds onto the stamp', async () => {
    ...
    expect(fs.stamped).toEqual([{ mode: 'read-only', egress: 'deny', workspaceRoot: resolve('/session-project') }])
  })
  ...
  it('an approved escalation stamps the granted mode onto that write', async () => {
    ...
    expect(fs.stamped).toEqual([{ mode: 'danger-full-access', egress: 'deny', workspaceRoot: resolve('/session-project') }])
  })
