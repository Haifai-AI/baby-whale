...
/** The settings namespace owning the user's per-skill enable state. */
export const SKILLS_SETTINGS_NAMESPACE = 'skills'

/** Modules-scoped guard so repeated calls register the namespace exactly once per context set. */
let skillsSettingsEnsured = false

/** Minimal structural view of the settings service used by the gate. */
interface SkillsSettingsHost {
  register(ns: string, schema: unknown): unknown
  get(ns: string): unknown
}

/** Any context that can surface services by name (the gateway's own view). */
interface SettingsHostContext {
  get(name: string): unknown
}

/**
 * Register the `skills` settings namespace on first need so the Settings UI
 * can toggle skills through the standard `settings.update` RPC and agents can
 * read the same resolved value through `settings.get`. Absent settings service
 * (headless compositions) leaves skill gating inert.
 * @param ctx - the host gateway context.
 */
function ensureSkillsSettings(ctx: SettingsHostContext): void {
  const settings = ctx.get('settings') as SkillsSettingsHost | undefined
  if (settings === undefined || skillsSettingsEnsured) return
  try {
    settings.register(SKILLS_SETTINGS_NS, skillsSettingsSchema)
    skillsSettingsEnsured = true
  } catch {
    // Duplicate registration only: another composition already owns it.
  }
}

/** Schemastery shape of the `skills` namespace user section. */
const skillsSettingsSchema = z.object({
  disabled: z.array(z.string()).default([]),
})

/** The branded settings namespace for the skills gate. */
const SKILLS_SETTINGS_NS = settingsNamespace(SKILLS_SETTINGS_NAMESPACE)

/**
 * Read the current disabled-skill name set from resolved settings.
 * @param ctx - any context sharing the settings service.
 * @returns names whose toggles are off; empty when settings are absent.
 */
function disabledSkillNames(ctx: { get(name: string): unknown }): ReadonlySet<string> {
  const settings = ctx.get('settings') as SkillsSettingsHost | undefined
  const value = settings?.get(SKILLS_SETTINGS_NAMESPACE) as
    | { disabled?: unknown }
    | undefined
  if (value === undefined || !Array.isArray(value.disabled)) return new Set()
  return new Set(value.disabled.filter((name): name is string => typeof name === 'string'))
}

/** Narrow local alias removed: helpers use structural interfaces above. */
...
export function createApiProxy(ctx: Context, defaults: ApiProxyDefaults): ApiProxy {
  ...
  return {
    ...
    skills: {
      ...
      async list(request) {
        ensureSkillsSettings(ctx)
        const { sessionId } = request.payload
        // Cold sessions resolve cwd from their stored header (the detached
        // history path); only a genuinely unknown id errors — the Settings →
        // Skills manager reads this before any agent has attached.
        const source = await historySourceFor(sessionId)
        const cwd = sourceSession(source).header.cwd
        if (cwd === undefined) {
        ...
        const scope = await presenterScopeFor(sessionId, sourceSession(source))
        try {
          // The Skills manager owns visibility: every discovered skill is
          // listed (model-invocable or not) so users can see and toggle all of them.
          const skills = await skillRegistry.list({ cwd, scope })
          const disabled = disabledSkillNames(ctx)
          return ok(request, {
            skills: skills.map(skill => ({
              ...
              userDisabled: disabled.has(skill.name),
            })),
          })
        } catch (error: unknown) {
    ...
    artifacts: {
      ...
      async list(request) {
        const { sessionId } = request.payload
        // The gallery is the DELIVERED set: only files the model claimed via
        // the deliver tool, read back from the session log (replay-safe,
        // order-stable, last-claim-wins per path). Nothing else — drafts and
        // working files stay out.
        const source = await historySourceFor(sessionId)
        const cwd = sourceSession(source).header.cwd
        if (cwd === undefined) {
          return err(request, { code: 'internal', message: `session "${sessionId}" has no workspace`, details: {} })
        }
        // Pair every deliver CALL with its RESULT: only successful claims
        // surface, keyed by the workspace-relative spelling so a retry with
        // an absolute path cannot duplicate an entry.
        ...
        for (const event of sourceSession(source).events) {
          ...
          for (const raw of paths) {
            ...
            claimed.delete(key)
            claimed.set(key, { seq: event.seq })
          }
        }
        if (claimed.size === 0) return ok(request, { artifacts: [] })
        ...
        return ok(request, { artifacts })
      },

      // Serve ORIGINAL bytes of one deliverables/uploads file. Jailed to the
      // session workspace's two artifact directories; content-type by ext.
      async raw(query, signal) {
        const source = await historySourceFor(query.sessionId)
        const cwd = sourceSession(source).header.cwd
        if (cwd === undefined) {
          return new Response(`session "${query.sessionId}" has no workspace`, { status: 409 })
        }
        const normalized = query.path.replaceAll('\\', '/')
      ...
      // Host-only GET channel: serve ONE cached preview PDF. The resolved
      // path must live inside the gateway-owned preview cache — this is not
      ...
      // Read-only bounded parse of one workspace artifact into the studio's
      // preview shapes. Path is joined onto the session cwd host-side; `..`
      // escape attempts fail resolution and never leave the workspace.
      async preview(request) {
        ...
        // series), and the LibreOffice single-page-per-sheet render for the
        // pixel-true "Original" tab. Everything degrades independently.
        if (ext === '.xlsx' || ext === '.xlsm') {
          ...
          // XlsxWriter/openpyxl write formulas without cached results; a
          // LibreOffice round-trip recomputes them so the Data grid shows
          // VALUES (the formula bar still carries the formula text).
          if (parsed !== undefined && xlsxHasUncachedFormulas(parsed) && soffice !== undefined) {
            const info = await stat(absolute)
          ...
          // Slides and documents have no browser-native studio: LibreOffice
          // renders the REAL file (charts, layouts, designs) to PDF and the
          // browser's native PDF viewer shows it; text parsing is the fallback.
          const soffice = findSoffice()
          if (soffice !== undefined) {
            const info = await stat(absolute)
    ...
    uploads: {
      // Intake twin of downloads: land user files into the session workspace
      // uploads/ dir. Session resolution mirrors skills.list (host-resident
      // header, no agent creation); the write stays inside <cwd>/uploads/.
      async workspaceFile(query, signal, request) {
        const session = ctx.sessions.get(query.sessionId)
        if (session === undefined) {
          return new Response(`session "${query.sessionId}" not found (not attached)`, { status: 404 })
        }
        ...
        if (Number.isFinite(declared) && declared > UPLOAD_MAX_BYTES) {
          return new Response('upload exceeds the 25 MB budget', { status: 413 })
        }
        let bytes: Uint8Array
        try {
          ...
          signal.throwIfAborted()
          return new Response(`failed to read upload body: ${error instanceof Error ? error.message : String(error)}`, { status: 400 })
        }
        signal.throwIfAborted()
        ...
        try {
          const outcome = await storeUpload(session.header.cwd, query.filename, bytes)
          return Response.json(outcome)
        } catch {
          // Never echo the absolute host path (or error text) back to the browser.
          return new Response('storing the upload failed', { status: 500 })
        }
      },
    },
