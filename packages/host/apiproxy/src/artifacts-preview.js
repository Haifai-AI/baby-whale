...
export function textPreviewKind(ext) {
...
export function mediaPreviewKind(ext) {
 ...
 */
export function parseMediaPreview(filePath) {
...
export function textPreviewLanguage(ext) {
...
function formulaOf(raw) {
    ...
    if (typeof record.sharedFormula === 'string')
        return `=${record.sharedFormula}`;
    return undefined;
}
/**
...
function cellView(raw) {
    ...
    if (raw instanceof Date)
        return { v: raw.toISOString().slice(0, 10) };
    if (typeof raw === 'object') {
        ...
        if (formula !== undefined)
            return { v: formula, f: formula };
        if (typeof record.text === 'string')
            return { v: record.text };
        if (Array.isArray(record.richText)) {
            return {
                v: record.richText.map((part) => typeof part === 'object' && part !== null && typeof part.text === 'string'
                    ? part.text
                    : '').join(''),
            };
        }
...
function formatByNumFmt(raw, numFmt) {
    ...
    if (numFmt.includes('%'))
        return `${(value * 100).toFixed(Math.min(decimals, 6))}%`;
 ...
 */
export async function loadWorkbookResilient(bytes) {
...
export async function parseXlsxPreview(bytes, fileName) {
    try {
        ...
        for (const ws of workbook.worksheets.slice(0, MAX_SHEETS)) {
            ...
            for (let r = 1; r <= scanRows; r++)
                grid.push(denseRow(ws.getRow(r).values, colCount));
            ...
            const nextContentRow = (from) => {
                for (let r = from + 1; r < grid.length; r++) {
                    ...
                    if (candidate !== undefined && !isDecoration(candidate))
                        return r;
                }
                return -1;
            };
            ...
            for (let r = 0; r < grid.length; r++) {
                const row = grid[r];
                if (row === undefined || isDecoration(row))
                    continue;
                if (fallbackIndex === -1)
                    fallbackIndex = r;
                if (isContent(row)) {
            ...
            if (headerIndex === -1)
                headerIndex = fallbackIndex === -1 ? 0 : fallbackIndex;
            ...
            if (availableRows > takeRows || ws.columnCount > MAX_COLS)
                truncated = true;
            const rows = Array.from({ length: takeRows }, (_, r) => denseCells(ws.getRow(startRow + r), colCount));
            if (headerText.length === 0 || rows.length === 0)
                continue;
            const sheet = {
                name: ws.name,
...
export function parsePptxPreview(bytes, fileName) {
    try {
        const entries = unzipSync(bytes);
        ...
        for (const name of slideNames) {
            ...
            for (const shape of shapes) {
                const paragraphs = [...shape.matchAll(/<a:p>[^]*?<\/a:p>/g)].map(paraMatch => [...paraMatch[0].matchAll(/<a:t(?:\s[^>]*)?>([^]*?)<\/a:t>/g)]
                ...
                for (const line of paragraphs) {
                    if (bullets.length < MAX_BULLETS)
                        bullets.push(cap(line, BLOCK_TEXT));
                }
            }
...
export function parseDocxPreview(bytes, fileName) {
    try {
        const entries = unzipSync(bytes);
        ...
        while ((match = blockRegex.exec(xml)) !== null && blocks.length < MAX_BLOCKS) {
            ...
            if (block.startsWith('<w:tbl')) {
                const rows = [...block.matchAll(/<w:tr[ >][^]*?<\/w:tr>/g)].map(row => [...row[0].matchAll(/<w:t(?:\s[^>]*)?>([^]*?)<\/w:t>/g)]
                    .map(t => decodeXmlEntities(t[1] ?? '').trim()).join(' '));
                const joined = rows.filter(r => r.replace(/\|/g, '').trim().length > 0).join('\n');
                if (joined.length > 0)
                    blocks.push({ type: 'table', text: cap(joined, BLOCK_TEXT) });
                continue;
            }
            ...
            if (text.length === 0)
                continue;
            let type = 'paragraph';
            if (styleAttr === 'heading1')
                type = 'heading1';
            else if (styleAttr === 'heading2')
                type = 'heading2';
            else if (styleAttr === 'heading3')
                type = 'heading3';
            else if (/quote/.test(styleAttr))
                type = 'quote';
...
function chartSheetNames(entries) {
    ...
    for (const [sheetPath, sheetName] of sheetParts) {
        ...
        for (const rel of decode(sheetRels).matchAll(/<Relationship\b[^>]*>/g)) {
            const { id, target } = relTarget(rel[0] ?? '');
            if (id === drawingRid && target !== undefined)
                drawingPath = joinTarget(sheetDir, target);
        }
        ...
        for (const rel of decode(drawingRels).matchAll(/<Relationship\b[^>]*>/g)) {
            ...
            if (target === undefined || !/\/chart\b/.test(/\bType="([^"]+)"/.exec(element)?.[1] ?? ''))
                continue;
            const chartName = basename(joinTarget(drawingDir, target));
            if (!map.has(chartName))
                map.set(chartName, sheetName);
        }
    }
    return map;
}
...
function parseChartXml(xml) {
    ...
    for (const serMatch of group.matchAll(new RegExp(`<${C}ser>[^]*?</${C}ser>`, 'g'))) {
        ...
        if (values.length === 0 && catRef === undefined && valRef === undefined)
            continue;
        series.push({
            ...(name !== undefined ? { name } : {}),
            categories,
 ...
 */
export async function parseXlsxCharts(bytes) {
    try {
        const entries = unzipSync(bytes);
 ...
 */
export function resetSofficeLookup() {
 ...
 */
export async function convertToPdfCached(soffice, sourcePath, cacheDir, sourceMtimeMs, filterSpec) {
    ...
    try {
        const info = await (await import('node:fs/promises')).stat(pdfPath);
    ...
    const convertWith = (spec) => new Promise((resolveConvert) => {
        execFile(soffice, [
            '--headless', '--norestore',
    ...
    const exists = async () => {
        try {
            return (await (await import('node:fs/promises')).stat(produced)).isFile();
        }
        catch {
            return false;
        }
    ...
    if (!await exists() && spec !== 'pdf')
        await convertWith('pdf');
    try {
        if (!await exists())
            return undefined;
        const target = `${cacheDir}/${key}.pdf`;
        if (produced !== target) {
            await (await import('node:fs/promises')).rename(produced, target);
        }
        return target;
    }
    catch {
 ...
 */
export function xlsxHasUncachedFormulas(preview) {
    if (preview.kind !== 'xlsx')
        return false;
    return preview.sheets.some(sheet => sheet.rows.some(row => row.some(cell => cell.f !== undefined && cell.v === cell.f)));
}
/**
 ...
 */
export async function recalcXlsxBytes(soffice, sourcePath, cacheDir, sourceMtimeMs) {
    const { createHash } = await import('node:crypto');
    const { mkdir, readFile, rename, stat } = await import('node:fs/promises');
    const { execFile } = await import('node:child_process');
    ...
    const done = await new Promise((resolveConvert) => {
        execFile(soffice, [
            '--headless', '--norestore',
            `-env:UserInstallation=file://${profile}`,
...
//# sourceMappingURL=artifacts-preview.js.map