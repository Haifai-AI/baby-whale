...
import { artifactsListRequestSchema, artifactsPreviewRequestSchema } from '../api/artifacts.schema.ts'
 ...
 */
// K ties the UNARY_ROUTES[K] row to its own schema/invoke pairing; a union
// key would degrade the row to an uninvokable intersection of every route.
...
export function toFetchHandler(api: ApiProxy): { fetch: typeof fetch } {
  return {
    ...
    async fetch(input: RequestInfo | URL, init?: RequestInit): Promise<Response> {
      ...
      if (path === '/api/artifacts.raw' && req.method === 'GET') {
        const parsed = artifactsRawQuerySchema.safeParse(Object.fromEntries(url.searchParams))
        if (!parsed.success) {
          return new Response('missing or invalid session/path query parameters', { status: 400 })
        }
        return api.artifacts.raw(
          {
      ...
      if (path === '/api/artifacts.file' && req.method === 'GET') {
        const parsed = artifactsFileQuerySchema.safeParse(Object.fromEntries(url.searchParams))
        if (!parsed.success) {
          return new Response('missing or invalid path query parameter', { status: 400 })
        }
        return api.artifacts.file(parsed.data, req.signal)
      }

      if (path === '/api/workspace.upload' && req.method === 'POST') {
        // Host-only intake twin of the download channel: raw octet-stream
        // bytes, query-addressed session. The JSON fence below must not see
        // this route, so it is answered before the envelope media check.
        const parsed = workspaceUploadQuerySchema.safeParse(Object.fromEntries(url.searchParams))
        if (!parsed.success) {
          return new Response('missing or invalid session/filename query parameters', { status: 400 })
        }
        ...
        return api.uploads.workspaceFile(
          { sessionId: parsed.data.session, filename: parsed.data.filename },
          req.signal,
          req,
        )
      }
