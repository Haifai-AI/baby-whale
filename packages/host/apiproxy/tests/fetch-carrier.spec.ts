...
function fakeApi(overrides: Partial<{ muxFrames: MuxFrame[]; hostFrames: HostFrame[]; crashOn: string }> = {}): ApiProxy {
  ...
  return {
    ...
    skills: {
      async list(request) {
        return { rpcId: request.rpcId, result: { ok: true, value: { skills: [{ name: 'commit-helper', description: 'Git commits', modelInvocable: true, userDisabled: false }] } } }
      },
    },
    goals: {
      async create(request) {
...
describe('unary round trip (handler ⇄ client, no network)', () => {
  ...
  it('round-trips skill.list through the wire form', async () => {
    ...
    expect(skills.result).toEqual({ ok: true, value: { skills: [{ name: 'commit-helper', description: 'Git commits', modelInvocable: true, userDisabled: false }] } })
  })
