...
| `startupTimeoutMs` | 两者 | 否 | 初始连接 + 工具同步的时限；到期即报告启动失败，由 `failOnStartupError` 决定致命还是仅记录日志（默认 30000） |
...
- 每个注册的定义都带有桥接私有的来源记录——公开名称背后的确切挂载身份 `{ serverName, rawName }`，保存在以定义对象本身为键的模块私有存储里，只能通过只读的 `getMcpToolOrigin()` 访问器读取。包不导出任何可写入或查询该存储的东西，因此同进程插件无法为自己的定义伪造归属；外形相似的对象（复制字段、重建的 symbol、原型技巧）只会解析为 `undefined`。消费方从这份来源解析工具归属，而不是解析公开名称：服务器 `a` 与 `a__b` 并存时，前缀解析会把 `mcp__a__b__t` 错误归给 `a__b`，过期的配置名还会窃取归属。每次再同步都会给新一代打记录，来源始终反映存活挂载。
...
- 启动受 `startupTimeoutMs` 约束：接受了连接却始终未完成初始同步的服务器（`initialize` 挂起、`tools/list` cursor chain 停滞）会在到期时被报告为启动失败，而不会无限期占用 fiber。在 `failOnStartupError` 下 fiber 拒绝并由 Cordis 回滚——关闭进行中的世代并终止已 spawn 的子进程——否则仅记录错误，监督器继续其（重）连接循环。
...
- **启动后的单请求超时仍继承自 MCP SDK**：启动完成后，除非设置了 `toolCallTimeoutMs`，每次工具调用都使用 SDK 的请求超时；会话中途缓慢劣化的服务器由重连预算约束，而非每阶段期限。
