...
- 每个注册的定义都带有来源戳（`MCP_TOOL_ORIGIN`：`{ serverName, rawName }`）——公开名称背后的确切挂载身份。消费方从这个戳解析工具归属，而不是解析公开名称：服务器 `a` 与 `a__b` 并存时，前缀解析会把 `mcp__a__b__t` 错误归给 `a__b`，过期的配置名还会窃取归属。每次再同步都会给新一代打戳，元数据始终反映存活挂载。
