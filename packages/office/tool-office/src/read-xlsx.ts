...
import { inferColumnType, shapeCellValue } from './read-shared.ts'
...
export interface ReadSheet {
  ...
  column_types: Array<{ col: string; kind: 'number' | 'formula' | 'text' }>
 ...
 */
export async function parseXlsxBytes(bytes: Uint8Array, maxRows: number = READ_MAX_ROWS): Promise<ReadWorkbook> {
  const workbook = await loadWorkbookResilient(bytes)
  if (workbook === undefined) throw new Error('unreadable workbook (exceljs failed, stripped-drawing retry also failed)')
  let truncated = false
  ...
  for (const ws of workbook.worksheets) {
    ...
    const firstRowValues = firstRow.values as unknown[]
    // exceljs row values are 1-indexed arrays; normalize to dense strings.
    const dense = (rowValues: unknown[], width: number): ReadCell[] =>
      Array.from({ length: Math.min(width, READ_MAX_COLS) }, (_, i) => shapeCellValue(rowValues[i + 1]))
    const colCount = Math.min(ws.columnCount, READ_MAX_COLS)
    if (ws.columnCount > READ_MAX_COLS) truncated = true
    const rowCount = ws.rowCount

    const rowOneCells = dense(firstRowValues, colCount)
    const secondRowValues = rowCount >= 2 ? ws.getRow(2).values as unknown[] : undefined
    ...
    for (let r = 0; r < takeRows; r++) {
      const values = ws.getRow(startRow + r).values as unknown[]
...
export function parseCsvText(text: string, maxRows: number = READ_MAX_ROWS): ReadSheet & { truncated: boolean } {
  ...
  const sampled = body.slice(0, maxRows).map(row =>
    Array.from({ length: Math.min(totalCols, READ_MAX_COLS) }, (_, c) => coerceCsvCell(row[c] ?? '')))
  return {
    name: 'csv',
...
function coerceCsvCell(token: string): ReadCell {
