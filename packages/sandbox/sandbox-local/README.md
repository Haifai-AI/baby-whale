...
Local implementation of the [`dsh-sandbox`](../sandbox/) seam. It selects one platform runner per call: Linux prefers a working `bwrap` then Landlock; macOS uses Seatbelt; Windows uses the ACL restricted-token runner. Multiple candidates are probed in order, while a sole candidate is selected directly. A network-deny policy additionally filters the chain to the rungs whose profiles confine egress (`bwrap`, Seatbelt); rungs without a network primitive fail closed with `SANDBOX_UNAVAILABLE` instead of leaving egress silently open.
...
- **Runner probes run once per provider lifetime** — per-rung verdicts are cached, so installing, removing, or repairing a runner requires reloading the plugin before selection changes; the egress filter itself reapplies on every call.
- **Landlock and Windows ACL are unavailable under network-deny** — neither rung confines egress, so deny policies fail closed with `SANDBOX_UNAVAILABLE` there. Deployments whose agent work needs the network set the `dsh-sandbox-policy` `egress: 'allow'` opt-in; unconfined `danger-full-access` executions bypass the sandbox either way.
- **`runnerCommand` is an operator assertion** — a configured custom runner skips functional probes and is assumed to implement the bwrap-compatible profile honestly, including the policy's egress posture; if it is itself a Bash script, its interpreter startup runs before that script applies confinement.
