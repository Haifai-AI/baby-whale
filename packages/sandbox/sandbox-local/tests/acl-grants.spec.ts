...
describe('windows-acl write grants (LocalSandboxProvider)', () => {
  // Allow posture throughout this suite: the windows-acl rung governs files
  // only, so a deny policy never reaches it (pinned in local.spec.ts). The
  // subject here is grant mechanics, not egress.
  ...
  it('deny fails closed before any ACL mutation: the files-only rung never sees the policy', async () => {
    try {
      const { sandbox } = await setup()
      const ws = workspaceRoot()
      scratch.push(ws)
      expect(() => sandbox.confine(
        ['pwsh', '/Command', 'x'],
        { mode: 'workspace-write', egress: 'deny', workspaceRoot: ws, sessionId: SessionId('denied') },
      )).toThrow(expect.objectContaining({ code: SANDBOX_UNAVAILABLE }))
  ...
  it('workspace-write materializes one standing workspace grant and one private temp capability, then reuses both', async () => {
    try {
      ...
      const ws = workspaceRoot()
      scratch.push(ws)
      const policy: SandboxPolicy = { mode: 'workspace-write', egress: 'allow', workspaceRoot: ws, sessionId: SessionId('sess-1') }
  ...
  it('read-only materializes no capability; upgrade creates them and downgrade leaves them reusable', async () => {
    try {
      ...
      const ws = workspaceRoot()
      scratch.push(ws)
      const readOnly: SandboxPolicy = { mode: 'read-only', egress: 'allow', workspaceRoot: ws, sessionId: SessionId('switch') }
      const workspaceWrite: SandboxPolicy = { mode: 'workspace-write', egress: 'allow', workspaceRoot: ws, sessionId: SessionId('switch') }
  ...
  it('a fresh provider gives a resumed session a new temp path and SID, so crash residue cannot collide', async () => {
    try {
      const ws = workspaceRoot()
      scratch.push(ws)
      const policy: SandboxPolicy = { mode: 'workspace-write', egress: 'allow', workspaceRoot: ws, sessionId: SessionId('resumed') }
  ...
  it('forks and workspace changes receive distinct temp capabilities while each workspace grant is reused', async () => {
    try {
      ...
      const parent = sandbox.confine(['true'], { mode: 'workspace-write', egress: 'allow', workspaceRoot: wsA, sessionId: SessionId('parent') })
      const child = sandbox.confine(['true'], { mode: 'workspace-write', egress: 'allow', workspaceRoot: wsA, sessionId: SessionId('child') })
      const moved = sandbox.confine(['true'], { mode: 'workspace-write', egress: 'allow', workspaceRoot: wsB, sessionId: SessionId('parent') })
  ...
  it('workspace grant failure disposes its SID, aggregates cleanup failure, and never creates a temp directory', async () => {
    try {
      ...
      expect(() => sandbox.confine(['true'], {
        mode: 'workspace-write', egress: 'allow', workspaceRoot: ws, sessionId: SessionId('workspace-fail'),
      ...
      expect(() => sandbox.confine(['true'], {
        mode: 'workspace-write', egress: 'allow', workspaceRoot: ws, sessionId: SessionId('workspace-cleanup-fail'),
  ...
  it('rejects a workspace containing the ambient temp root before any ACL mutation', async () => {
    const { sandbox } = await setup()
    expect(() => sandbox.confine(['true'], {
      mode: 'workspace-write', egress: 'allow', workspaceRoot: realpathSync.native(tmpdir()), sessionId: SessionId('overlap'),
  ...
  it('temp grant creation/add failures remove the random directory; cleanup failures aggregate', async () => {
    try {
      ...
      expect(() => sandbox.confine(['true'], {
        mode: 'workspace-write', egress: 'allow', workspaceRoot: ws, sessionId: SessionId('create-fail'),
      ...
      expect(() => sandbox.confine(['true'], {
        mode: 'workspace-write', egress: 'allow', workspaceRoot: ws, sessionId: SessionId('add-fail'),
      ...
      expect(() => sandbox.confine(['true'], {
        mode: 'workspace-write', egress: 'allow', workspaceRoot: ws, sessionId: SessionId('rm-fail'),
      ...
      expect(() => sandbox.confine(['true'], {
        mode: 'workspace-write', egress: 'allow', workspaceRoot: ws, sessionId: SessionId('aggregate-fail'),
      })).toThrow(/temp grant materialization failed and its cleanup also failed/u)
    } finally {
      cleanup()
    }
  ...
  it('agentless calls pass a temp root and no capabilities; the runner owns the private child lifecycle', async () => {
    try {
      const { sandbox, fiber } = await setup()
      const confined = sandbox.confine(['pwsh', '/Command', 'x'], { mode: 'workspace-write', egress: 'allow', workspaceRoot: '/ws' })
      expect(confined.argv).toEqual([
        'node', 'windows-acl-runner.js',
        '--workspace', '/ws',
        '--temp', tmpdir(),
  ...
  it('provider teardown reports grant and directory cleanup failures without aborting teardown', async () => {
    try {
      ...
      const confined = sandbox.confine(['true'], {
        mode: 'workspace-write', egress: 'allow', workspaceRoot: ws, sessionId: SessionId('dispose'),
      })
      const tempDir = flag(confined.argv, '--temp') ?? ''
