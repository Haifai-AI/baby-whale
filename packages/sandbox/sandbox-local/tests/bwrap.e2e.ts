...
const probe = spawnSync('bwrap', [...bwrapProfileArgs({ mode: 'read-only', egress: 'deny', workspaceRoot: '/' }), '--', 'true'], { timeout: 5_000, stdio: 'ignore' })
...
describe.skipIf(!bwrapUsable)('sandbox-local: real bwrap confinement', () => {
  it('the passing probe selects the bwrap rung naturally — first in the ladder, full enforcement, EROFS dialect', async () => {
    ...
    const confined = sandbox.confine(['true'], { mode: 'read-only', egress: 'deny', workspaceRoot: workdir })
  ...
  it('read-only denies a write — the file must NOT exist, and the kernel speaks the advertised dialect', async () => {
    const workdir = await tempDir(tmpdir())
    const sandbox = await provider()
    const { result } = runConfined(sandbox, `echo hi > ${workdir}/denied.txt`, { mode: 'read-only', egress: 'deny', workspaceRoot: workdir })
  ...
  it('read-only keeps the tree readable/executable and the fresh /dev/null writable', async () => {
    const workdir = await tempDir(tmpdir())
    const sandbox = await provider()
    const { result } = runConfined(sandbox, 'ls / > /dev/null && echo dev-ok', { mode: 'read-only', egress: 'deny', workspaceRoot: workdir })
  ...
  it.each(['read-only', 'workspace-write'] as const)(
    ...
    async (mode) => {
      ...
      const visibility = runConfined(sandbox, 'readlink /proc/self/ns/pid', { mode, egress: 'deny', workspaceRoot: workdir })
      ...
      const escape = runConfined(
        ...
        { mode, egress: 'deny', workspaceRoot: workdir },
      )
  ...
  it('keeps descendants observable and controllable inside the private PID namespace', async () => {
    ...
    const { result } = runConfined(
      ...
      { mode: 'read-only', egress: 'deny', workspaceRoot: workdir },
    )
    expect(result.status).toBe(0)
  })
  ...
  it('workspace-write lands a write inside the workspace root and still denies one beside it', async () => {
    ...
    const inside = runConfined(sandbox, `printf bwrap-ok > ${workdir}/allowed.txt`, { mode: 'workspace-write', egress: 'deny', workspaceRoot: workdir })
    expect(inside.result.status).toBe(0)
    expect(readFileSync(join(workdir, 'allowed.txt'), 'utf8')).toBe('bwrap-ok')

    const denied = runConfined(sandbox, `echo hi > ${outside}/denied.txt`, { mode: 'workspace-write', egress: 'deny', workspaceRoot: workdir })
  ...
  it('workspace-write mounts an EPHEMERAL /tmp: the write succeeds inside, the host /tmp stays untouched', async () => {
    ...
    const sandbox = await provider()
    const { result } = runConfined(sandbox, `printf tmp-ok > ${target} && cat ${target}`, { mode: 'workspace-write', egress: 'deny', workspaceRoot: workdir })
    expect(result.status).toBe(0)
