...
describe.skipIf(!landlockUsable)('sandbox-local: real Landlock confinement through the bundled launcher', () => {
  it('read-only denies a write — the file must NOT exist, the wrap reports the probed enforcement', async () => {
    ...
    const { result, enforcement: wrapped } = runConfined(sandbox, `echo hi > ${workdir}/denied.txt`, { mode: 'read-only', egress: 'allow', workspaceRoot: workdir })
  ...
  it('read-only keeps the tree readable/executable and /dev/null writable', async () => {
    const workdir = await tempDir(tmpdir())
    const sandbox = await provider()
    const { result } = runConfined(sandbox, 'ls / > /dev/null && echo dev-ok', { mode: 'read-only', egress: 'allow', workspaceRoot: workdir })
  ...
  it('read-only denies a write beneath the host /dev (the /dev/shm tmpfs must stay untouched)', async () => {
    ...
    const { result } = runConfined(sandbox, `echo hi > ${target}`, { mode: 'read-only', egress: 'allow', workspaceRoot: workdir })
    expect(result.status).not.toBe(0)
  ...
  it('workspace-write lands a write inside the workspace root and still denies one beside it', async () => {
    ...
    const inside = runConfined(sandbox, `printf landlock-ok > ${workdir}/allowed.txt`, { mode: 'workspace-write', egress: 'allow', workspaceRoot: workdir })
    expect(inside.result.status).toBe(0)
    expect(readFileSync(join(workdir, 'allowed.txt'), 'utf8')).toBe('landlock-ok')

    const denied = runConfined(sandbox, `echo hi > ${outside}/denied.txt`, { mode: 'workspace-write', egress: 'allow', workspaceRoot: workdir })
  ...
  it('workspace-write grants the host /tmp (the documented Landlock-profile difference)', async () => {
    ...
    const sandbox = await provider()
    const { result } = runConfined(sandbox, `printf tmp-ok > ${scratch}/scratch.txt`, { mode: 'workspace-write', egress: 'allow', workspaceRoot: workdir })
    expect(result.status).toBe(0)
