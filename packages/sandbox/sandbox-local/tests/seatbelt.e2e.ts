...
const probe = spawnSync('sandbox-exec', [...seatbeltProfileArgs({ mode: 'read-only', egress: 'deny', workspaceRoot: '/' }), '--', 'true'], { timeout: 5_000, stdio: 'ignore' })
...
describe.skipIf(!seatbeltUsable)('sandbox-local: real Seatbelt confinement through sandbox-exec', () => {
  it('read-only denies a write — the file must NOT exist, and the kernel speaks the advertised dialect', async () => {
    ...
    const { result, confined } = runConfined(sandbox, `echo hi > ${workdir}/denied.txt`, { mode: 'read-only', egress: 'deny', workspaceRoot: workdir })
  ...
  it('read-only grants no temp area: a write under the user temp dir is denied too', async () => {
    ...
    const { result } = runConfined(sandbox, `echo hi > ${target}`, { mode: 'read-only', egress: 'deny', workspaceRoot: await tempDir(homedir()) })
    expect(result.status).not.toBe(0)
  ...
  it('workspace-write lands a write inside the workspace root and still denies one beside it', async () => {
    ...
    const inside = runConfined(sandbox, `printf seatbelt-ok > ${workdir}/allowed.txt`, { mode: 'workspace-write', egress: 'deny', workspaceRoot: workdir })
  ...
  it('workspace-write grants /tmp and the user temp dir (the documented Seatbelt-profile temp areas)', async () => {
    ...
    const { result } = runConfined(
      ...
      { mode: 'workspace-write', egress: 'deny', workspaceRoot: workdir },
    )
    expect(result.status).toBe(0)
