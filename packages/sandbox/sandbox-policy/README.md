...
- `egress` — the deployment network-egress posture (`deny` / `allow`) for confined executions, validated at load. Default `deny`: a file sandbox is not an exfiltration channel. There is no per-call override.
...
- `ctx.sandboxPolicy.defaultMode` / `ctx.sandboxPolicy.defaultEgress` / `ctx.sandboxPolicy.workspaceRoot` — the deployment default, egress posture, and fallback root used by `resolve()`.
...
- **File-effect modes plus one egress posture** — `SandboxMode` governs file effects and process visibility stays outside the vocabulary; network egress is the separate `egress` posture (default `deny`), resolved from deployment config with no per-call override.
