...
class TestFileSystem extends FileSystem {
  ...
  override async writeBytes(target: FsTarget, bytes: Uint8Array): Promise<FsBinaryWriteOutcome> {
    await mkdir(dirname(target.displayPath), { recursive: true })
    await writeFile(target.displayPath, bytes)
    return { operation: 'create', version: FsVersion('test'), size: bytes.byteLength }
  }
