...
export function apply(ctx: Context, config: Config = {}): void {
  ...
  const skillTool = defineTool({
    ...
    async execute(args, exec) {
      ...
      if (userDisabledSkillNames(ctx).has(args.name)) {
        throw new Error(`skill "${args.name}" is disabled by the user in Whale settings`)
      }
  ...
  /** Minimal structural view of the settings service used by the skill gate. */
  interface SettingsGateHost {
    get(ns: string): unknown
  }

  /**
 * The user's disabled-skill names, read per evaluation so Settings toggles
 * apply without remounting any session realm. Absent settings (headless or
 * older compositions) means nothing is disabled.
 */
  function userDisabledSkillNames(ctx: { get(name: string): unknown }): ReadonlySet<string> {
    const settings = ctx.get('settings') as SettingsGateHost | undefined
    const value = settings?.get('skills') as { disabled?: unknown } | undefined
  ...
  ctx.on('agent/pre-step', async (
    ...
    for (const name of names) {
      ...
      if (userDisabledSkillNames(ctx).has(name)) continue
  ...
  ctx.on('agent/pre-step', async (
    ...
    const disabled = userDisabledSkillNames(ctx)
    const skills = snapshot.skills.filter(skill => isModelInvocable(skill) && !disabled.has(skill.name))
