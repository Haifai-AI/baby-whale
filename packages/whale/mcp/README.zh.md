...
- `mcpStatus.list()` 把管理器状态（`connecting` / `connected` / `failed` / `stuck` / `disabled`）与调用时从注册表实时读取的工具合并——桥接重新同步（`list_changed`、重连）后无需推送信道即可反映。工具归属是精确的，绝不从公开名称反推：桥接注册的每个定义都带有来源戳（`MCP_TOOL_ORIGIN`：挂载方服务器名加原始线上名称），每一行只列出自己挂载注册的工具——公开的 `toolNames` 加上可直接展示的 `localToolNames`。前缀解析不可能做到严密：服务器 `a` 与 `a__b` 同时配置时，公开名称 `mcp__a__b__t`（服务器 `a` 上的原始名 `b__t`）同样匹配 `a__b` 的命名空间，停用或过期的配置名还会窃取归属。
...
- 首次使用审批：`tools/pre-execute` 栅栏在会话首次调用每个 MCP 工具前发问（`运行 MCP 工具 "mcp__x__y"（来自服务器 "x"）？`）。服务器子句来自挂载来源戳——与状态分组同一套精确归属；会话审计日志即记忆——每工具每会话一次 allowed-once 授权；处于 never-prompt 策略（danger-full-access）的会话不设栅栏，与 whale guardrails 的覆盖写栅栏一致。
